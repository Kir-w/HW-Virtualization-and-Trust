...
Vous devez vérifier si le FIFO est prêt à envoyer ou à recevoir des données en lisant les bits du registre UART_STATUS.*/use core::ptr::{read_volatile, write_volatile};
...
impl Usart {
    ...
    pub fn receive() -> u8 {
        unsafe {
            ...
            while read_volatile(UART_STATUS) & (1 << 0) == 0 {}

            // Lire les données depuis le FIFO
            read_volatile(UART_FIFO) as u8
        }
    }
}
